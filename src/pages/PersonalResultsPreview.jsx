...
const DEMO = {
  ratee: { user_name: 'Alex M.' },
  scores: { leader_score: 9.20, manager_score: 8.40, ic_score: 8.10 },
  percentiles: { total_pct: 88 },
  top5: [
    { name: 'Vision' }, { name: 'Influence' }, { name: 'Courage' },
    { name: 'Integrity' }, { name: 'Storytelling' }
  ],
  bottom5: [
    { name: 'Consistency' }, { name: 'Planning' }, { name: 'Discipline' },
    { name: 'Time Management' }, { name: 'Coordination' }
  ]
};
...
export default function PersonalResultsPreview() {
  ...
  const dims = [
    { key: 'leader', label: 'Leader', score: DEMO.scores.leader_score, isTop: true },
    { key: 'manager', label: 'Manager', score: DEMO.scores.manager_score, isTop: false },
    { key: 'ic', label: 'Contributor', score: DEMO.scores.ic_score, isTop: false },
  ];
  ...
  return (
    <div className="pr-bg">
      ...
      <div className="pr-wrapper">
        <div style={{ background: 'rgba(239,159,39,0.1)', border: '1px solid rgba(239,159,39,0.3)', borderRadius: '10px', padding: '0.5rem 1.2rem', fontSize: '0.75rem', color: '#ef9f27', marginBottom: '1rem', textAlign: 'center' }}>
        ...
        <div className="pr-hero">
          ...
          <p className="pr-name">{DEMO.ratee.user_name}</p>
        </div>
        <div className="pr-scores">
          {dims.map(d => <ScoreRing key={d.key} score={d.score} color={type.color} label={d.label} isTop={d.isTop} />)}
        </div>
        <div className="pr-percentile-card" style={{ borderColor: type.border, boxShadow: `0 0 40px ${type.glow}` }}>
          ...
          <div className="pr-percentile-value" style={{ color: type.color }}>Top 12%</div>
        ...
        <div className="pr-attributes">
          <div className="pr-attr-col">
            ...
            {DEMO.top5.map((a, i) => (
              <div className="pr-attr-tag pr-attr-top" key={i} style={{ background: type.glow, borderColor: type.border, color: type.color }}>{a.name}</div>
            ))}
          </div>
          <div className="pr-attr-col">
            <div className="pr-attr-title" style={{ color: 'rgba(255,255,255,0.35)' }}>↓ Growth Areas</div>
            {DEMO.bottom5.map((a, i) => <div className="pr-attr-tag pr-attr-bottom" key={i}>{a.name}</div>)}
          </div>
        </div>
